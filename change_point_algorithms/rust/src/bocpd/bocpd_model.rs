...
impl BocpdModel {
    #[new]
    pub fn new_py(alpha: f64, beta: f64, mu: f64, kappa: f64, with_cache: bool, threshold: Option<f64>) -> PyResult<Self> {
        let initial = NormalInverseGamma {alpha, beta, mu, kappa};
        ...
        Ok(Self {
            ...
            threshold, prev_max, curr_max, probs, params,
            beta_cache: cache})
    }
    ...
    pub fn predict(&mut self, point: f64) -> f64 {
        ...
        // dot product
        zip(priors.iter(), self.probs.iter()).map(|(change, prob)| {
    ...
    fn truncate_vectors(&mut self) -> usize {
        let mut threshold_filter: Vec<bool> = self
            .probs
            .iter()
            .map(|prob| prob.get_value() >= self.threshold).collect();
    ...
    fn update_params(&mut self, point: f64) {
        ...
        let NormalInverseGamma {alpha, beta, mu, kappa} = self.initial_params;
